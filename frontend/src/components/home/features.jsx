...
const Features = ({ location }) => {
  ...
  // STRICT FRONTEND FILTER: Only keep items where featured is true
        ...
        <div className="flex items-end justify-between gap-2 w-full px-4 sm:px-0">
          <div className="space-y-1 lg:space-y-2">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 lg:px-4 lg:py-1.5 rounded-full bg-primary/10 text-primary border border-primary/20 text-[9px] lg:text-xs font-black uppercase tracking-wider">
              <Sparkles size={10} fill="currentColor" className="lg:w-3 lg:h-3" /> Featured
            </div>
        ...
        >
          {isLoading ? (
            Array(4).fill(0).map((_, i) => (
              <div key={`feat-skel-${i}`} className="snap-start shrink-0 w-[170px] sm:w-[220px] lg:w-[300px]">
                <CardSkeleton />
              </div>
            ))
          ) : (
